...
class DHT:
    ...
    def read(self):
        ...
        # print "1: %d 2: %d 3: %d 4: %d" % (the_bytes[0], the_bytes[1], the_bytes[2], the_bytes[3])
        if not (self.__is_DHT22()):
            return DHTResult(DHTResult.ERR_NO_ERROR, the_bytes[2], the_bytes[0])
        else:
            (temperature, humidity) = self.__DHT22_values(float((the_bytes[2] * 2 + the_bytes[3])) / 10, (the_bytes[0] * 256 + the_bytes[1]) / 10)
            return DHTResult(DHTResult.ERR_NO_ERROR, temperature, humidity)


    def __DHT22_values (self, temperature, humidity):
        if (temperature > 0x7fff):
            temperature = 0x8000 - temperature
        return (temperature, humidity)

    def __is_DHT22(self):
        return self.__sensor == 'DHT22'
